...
export function semanticTree(doc: ButterDocument) {
  const configs = readSemantics(doc.root, doc.semantics), nodes: DeclaredSemanticNode[] = []
  function visit(node: Widget, parent: string | null) {
    ...
    const attributes: Record<string, string> = { id: config.id, label: config.label ?? node.name }
