...
import { emitButter } from './model/butter.ts'
...
export default function App() {
  ...
  const compiled = useMemo(() => {
    try {
      return { spec: compileSpec(root), template: emitButter(root), error: null }
    } catch (error) {
      return { spec: null, template: '', error: error instanceof Error ? error.message : 'invalid tree' }
    }
  }, [root])
  ...
  return (
    <div className="h-svh flex flex-col bg-background text-foreground">
      <header className="flex items-center gap-3 px-4 py-2 border-b border-white/10">
        <h1 className="text-sm font-medium">Butter GUI Builder</h1>
      ...
      <div className="flex-1 grid grid-cols-[280px_1fr_320px] min-h-0">
        ...
        <Surface className="p-3 overflow-auto rounded-none border-l border-white/10">
          {compiled.spec ? (
            <ExportPanel spec={compiled.spec} template={compiled.template} />
          ) : (
